...
/*
		...
		[2-6] 다음 중 키워드가 아닌 것은?(모두 고르시오) b,c,d,e (모든 키워드는 소문자)
				a. if
				b. True
				c. NULL
				d. Class
	    ...
	    [2-8] 참조형 변수(reference type)와 같은 크기의 기본형(primitive type)은? (모두 고르시오) a,d (4byte)
				a. int
				b. long
				c. short
				d. float
		...
		[2-9] 다음 중 형변환을 생략할 수 있는 것은? (모두 고르시오) d,e
				...
				int i = 100;
				long l = 1000L;
				a. b = (byte)i; //int(4) -> byte(1)
				b. ch = (char)b; //byte(1) -> char(2)숫자를 문자로 바꿈
				c. short s = (short)ch; //char(2) -> short(2) 문자를 숫자로 바꿈
				d. float f = (float)l; //long(8) -> float(4)이지만 float이 더 큼
				e. i = (int)ch; //int보다 작은 크기를 가진것은 int로 자동형변환됨
	    ...
	    [2-11] 다음중 변수를 잘못 초기화 한 것은? (모두 고르시오) a,b,c,d
	    				a. byte b = 256; ~127
	    				b. char c = ''; ->char c ='c';
	    				c. char answer = 'no'; ->String answer = "no";
	    				d. float f = 3.14 -> float f = 3.14f
	    				e. double d = 1.4e3f; ->double이 더 커서 가능?
